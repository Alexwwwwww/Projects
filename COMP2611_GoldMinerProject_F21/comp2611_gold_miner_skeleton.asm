#	SID: 20761210
#	Yumeng Wang
#	ywanglu@connect.ust.hk
#	COMP2611 Course Project
...
# get timer: $t2
 		la   $t2, level_timer
 		lw   $t2, 0($t2)
 		# level_balance>=level_quota
 		bge  $t0, $t1, status_win
 		# not win
 		# loose
 		blez $t2, status_lose
 		# not end
...
#	| RectB.botrigt_x |
#	| RectB.botrigh_y | <-- $sp 		
		#get the location of the mineral

		# store the s registers used by its caller
		addi $sp, $sp, -4
		sw $s0, 0($sp)
		addi $sp, $sp, -4
		sw $s1, 0($sp)
		...
		# return whenever it is an invalid mineral
		la   $t0, mineral_ids
		sll  $t1, $a0, 2
		add  $t0, $t1, $t0
		lw   $t0, 0($t0)  # now $t0 is the id

		# see if the id is zero
        beq $t0, $zero, exit_func
		# add  $s2, $t0, $a0   # get id
		
		move $s2, $t0	#		$s2 is the index of the mineral  
		# $a0 is already passed to the function
		
		move $a0, $s2
		li   $v0, 108
		syscall
		# save location of the mineral in $s0=x, $s1=y
		move $s0, $v0
		move $s1, $v1

		# get the size of the mineral
		# get the level of the game: $s3
		# deduce the mineral type
		# $a0<=1+3*$s3 --->$s6
		# is gold
		# find the id according to the numbe of the mineral allowed in current round
		la   $s6, mineral_num
		lw   $s6, 0($s6)
		...
		addi $t2, $s6, 1 # this is the index
		bgt  $s2, $t2, gem_intv
		li   $s4, 60
		li   $s5, 45
		j    mineral_x_y
		# $a0>1+2*$s3 && $a0<=1+5*$s3 --->$t1
gem_intv:		
		add  $t1, $t2, $t1    # 4*$s3+$s3
		bgt  $s2, $t1, rock_intv 	# $s2<=$s6
		li   $s4, 30
		li   $s5, 18
		j    mineral_x_y
rock_intv:
		# $a0>1+5*$s3 && $a0<=1+9$s3 --->$t2
		li   $s4, 100
		li   $s5, 75
		j    mineral_x_y
		# $a0>1+9*$s3 --->$t3
mineral_x_y:	
		sub  $t0, $s0, $zero #	left up x
		sub  $t1, $s1, $zero #	left up y
		add  $t2, $s0, $s4 #	right down x
		add  $t3, $s1, $s5 #	right down y
		
		#get the location of the hook
		li   $a0, 1
		li   $v0, 108
		syscall
		...
		addi $t6, $v0, 22
		addi $t7, $v1, 32
		# save them in to stack
		# addi $sp, $sp, -4
		# sw   $s2, 0($sp)
		addi $sp, $sp, -36     # save to the stack
		sw   $t7, 0($sp)
		sw   $t6, 4($sp)
		sw   $t5, 8($sp)
		sw   $t4, 12($sp)
		...
		sw   $t0, 28($sp)
		sw   $ra, 32($sp)
		# call is_intesect function 
		jal  check_intersection
		# $v0=1 is hit, also the return value here, the same return value
		# now restore the $ra
		lw   $ra, 32($sp)
		# restore the stack
		addi $sp, $sp, 36
...
# ============================= You codes start here ================================
		# updating level balance, $a0 is the obj index
		addi $sp, $sp, -4
		sw   $s0, 0($sp)
		addi $sp, $sp, -4
		sw   $s1, 0($sp)
		...
		sll $t0, $a0, 2
		add $t0, $t0, $t3 # get id
		lw  $s0, 0($t0)#save the id in $s0
		beqz $s0, exit_update

		move $a0, $s0 
		li $v0, 116
		syscall
		# now $v0 is the price of the mineral
		la $t2, level_balance
		lw $t1, 0($t2) # get old balance
		add $t1, $t1, $v0 # balance ($s0)= balance ($s0) + price ($v0)
		sw  $t1, 0($t2)
		# pass the price
		add $a1, $t1, $zero # pass new balance to $a1
		li  $a0, 3 # 3 means balance
		li  $v0, 103 # 103 syscall
		syscall
		# destroying the mineral
		move $a0, $s0  # set the id
		li $v0, 107
		syscall
		# demolish the id from the array
		# store the adress of the object array insice $s1

		
		sw $zero, 0($t0) # set it to zero
...
# ============================= You codes start here ================================
#	$t0: i, loop through all the index of minerals
#      $s0: id of the dynamite
#	$s3 game level
#	| RectA.topleft_x |
#	| RectA.topleft_y |
#	| RectA.botrigt_x |
...
#	| RectB.botrigh_y | <-- $sp
		...
		move $s0, $a0	#$s0=$a0
		li   $t0, 0   # $t0 is the index, mineral index from 0, loop all the index (index is cumulative)
dynamite_Loop: 		
		#check exit condition: larger or equal than the largest index allowed
		# la  $t6, game_level
		# lw  $t6, 0($t6) # get the current level
		# mul $t7, $t6, 9 # $t7= 9*$t6
		la $t6, mineral_num
		lw $t7, 0($t6)
		# addi $t7, $t7, 1
	
		addi $t7, $t7, -1 # max index of mineral
		bgt  $t0, $t7, check_dynamite_hit_end # if larger than the max index of the mineral exit loop
# if the id is zero, continue
		la  $t1, mineral_ids
		sll $t2, $t0, 2
		add $t1, $t1, $t2
		lw  $a0, 0($t1) # id of the mineral
		
		beqz $a0, cond_loop 
# get the dynamite location
		li   $v0, 108
		move $a0, $s0 #dynamite id
		syscall

		move $s1, $v0 # x of the dynamite into $s1
		move $s2, $v1 # y of the dynamite into $s2
		# push the coordinate of the dynamite into stack
		
		# first put $ra into stack
...
#=====================================================================================
		...
		li   $v0, 108
		syscall
		# deduce the type of the mineral
		# according to the index num $s0
		# save location of the mineral in $s1=x, $s2=y
		move $s1, $v0
		move $s2, $v1	
		# get the size of the mineral
		# get the level of the game: $s3
		...
		# $a0<=1+2*$s3 --->$t0
		# is gold
		addi  $t1, $t1, -1 #	-1+gold_num= index number max
		bgt  $t0, $t1, gem_intv_1 # INDEX larger than the largest index of gold
		li   $s4, 60
		li   $s5, 45
		j    mineral_x_y_1
		# $a0>1+2*$s3 && $a0<=1+5*$s3 --->$t1
...
mineral_x_y_1:	
		...
		addi $sp, $sp, 4
		# if is intersected
		# let $t5 to temporarily be the threshold
		# not intersect $v0=0
		beqz $v0, cond_loop
		# is intersected, destroy the mineral
		# move  $a0, $t0 # set the id
		...
		sw  $zero, 0($t1) # set it to zero
		j check_dynamite_hit_end_1
#==================================to be modified=====================================		
#=====================================================================================
cond_loop:
		addi $t0, $t0, 1
...
check_dynamite_hit_end:
		...
		addi $sp, $sp, 4
 		...
 		li   $v0, 0 # no hit
 		jr   $ra
#	deal with successful destroy
 check_dynamite_hit_end_1:
		lw   $s7, 0($sp)
		addi $sp, $sp, 4
		lw   $s6, 0($sp)
		addi $sp, $sp, 4
		...
		li   $v0, 1 # loop is not over but can break since is hit
		jr   $ra
# ============================= Your codes end here =================================
...
# 	Thirdly, set the value of $v0 based on the check result and jump to return address. (logic: or)
# ============================= You codes start here ================================
		#load the data
#	| RectA.topleft_x |	| RectA.topleft_y |	| RectA.botrigt_x |	| RectA.botrigh_y |
#	$s0					$s1					$s2					$s3
#	| RectB.topleft_x |	| RectB.topleft_y |	| RectB.botrigt_x |	| RectB.botrigh_y | <-- $sp 
#	$s4					$s5					$s6					$s7
		#push
		...
		lw   $s1, 56($sp)
		lw   $s0, 60($sp)
#		============== seems no need to update the stack pointer??=========
# 		condition1: whether recA's left edge is to the right of recB's right edge;
		sgt  $t0, $s0, $s6
# 		condition2: whether recA's right edge is to the left of recB's left edge;
		slt  $t1, $s2, $s4
		or   $t0, $t1, $t0 #intermediate result
# 		condition3: whether recA's top edge is below recB's bottom edge;
		sgt  $t1, $s1, $s7
# 		condition4: whether recA's bottom edge is above recB's top edge.
		slt  $t2, $s3, $s5
		or   $t1, $t1, $t2 #intermediate result
		
		or  $t0, $t0, $t1 # if t0=1, no intersected
		slti $t0, $t0, 1   # if t0==1, not less than 1, $t0=0, not intersect
				    # if t0==0, is less than 1, $t0=1, intersect
...
# ============================= You codes start here ================================
		...
		blt  $v0, -85, toggle_hook_direction_1  # in the counter-clockwise direction, if it is less than -85
		bgt  $v0, 85, toggle_hook_direction_2
		#otherwise, maintain original direction
		li   $v0, 122
 		li   $a0, 1
 		syscall # get the status
 		# if isClockwise==1
 		beq  $v0, 1, move_clockwise
move_counter_clockwise:
		li   $v0, 115 # change direction
 		li   $a0, 1 # id of hook
 		li   $a1, 3 # swinging delta degree: 3 means counter-clockwise movement
 		syscall
 		j    mh_exit # exit
...
move_clockwise:
		...
		syscall
		j    mh_exit
		#if angle<-85: data in $a1 is possitive
		#if angle>85: data in $a2 is negative
toggle_hook_direction_1:
		li  $a0, 1
		li  $v0, 123
		syscall
		j  move_counter_clockwise
toggle_hook_direction_2:
		li  $a0, 1
		li  $v0, 123
		syscall
		j  move_clockwise
